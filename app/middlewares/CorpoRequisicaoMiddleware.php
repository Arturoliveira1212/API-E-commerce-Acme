...
class CorpoRequisicaoMiddleware
{
    ...
    private function validarCorpoRequisicao(array $corpoRequisicao, array &$erros = [])
    {
        ...
        if (! empty($this->camposObrigatorios)) {
            $this->validarCampos($this->camposObrigatorios, $corpoRequisicao, $corpoRequisicaoValidado, $erros, true);
        }

        if (! empty($this->camposOpcionais)) {
            $this->validarCampos($this->camposOpcionais, $corpoRequisicao, $corpoRequisicaoValidado, $erros, false);
        }
        ...
        foreach ($campos as $campo => $tipo) {
            ...
            if (is_array($tipo)) {
                if (is_array($corpoRequisicao[ $campo ])) {
                    ...
                    $this->validarCampos($tipo, $corpoRequisicao[ $campo ], $corpoRequisicaoValidadoFilho, $errosFilho, $campoObrigatorio);
                    if (! empty($errosFilho)) {
